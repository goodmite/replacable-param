...
export const MsgBox = () => {
  ...
  wrapper.addEventListener("keypress", (e) => {
    let el = $(e.target);
    if (
      e.target.classList.contains("spnhighlight") &&
      e.target.classList.contains("pristine")
    ) {
      el.text("");
  ...
  return {
    ...
    getTxt: function (flag) {
      ...
      var finalHtml = "";
      var i;
      for (i = 0; i < $(t[0]).find("div").length - 1; ++i) {
        finalHtml += $($(t[0]).find("div")[i]).html() + "\n";
      }
      finalHtml += $($(t[0]).find("div")[i]).html();
      return finalHtml;
    },
    ...
    set2span: function (e, ctrl) {
      const ctrlspan = $(
        wrapper.querySelector(labelFroalaEditor).querySelector("iframe")
      )
        .contents()
        .find("#" + $(ctrl).attr("id").substring(3));
      $(ctrl).hide();
      ctrlspan.show();
      if ($(ctrl).val() !== undefined && $(ctrl).val().length > 0) {
        ...
        // checks wheather '<' and '>' are present in the replceable params value or not, checks send button throws error not not by clicking
          ...
          ctrlspan.text().search("<") !== -1 &&
          ctrlspan.text().search(">") !==
            -1 /*&& environment.sendButtonClicked*/
        ) {
        ...
        case 9:
          {
            ...
            if (e.type === "keydown") {
              this.showtxtbox(
                $(
                  wrapper
                    .querySelector(labelFroalaEditor)
                    .querySelector("iframe")
                )
                  .contents()
                  .find("#spn" + nextItemIndex)[0]
              );
    ...
    parseMessage: function (msg) {
      ...
      if (result !== undefined && result !== null) {
        for (let i = 0; i < result.length; i++) {
          ...
          let ele = `<span contenteditable='false'class='outerspan'><span  title='${text}' contenteditable='true' id='spn${i}' class='spnhighlight highlight pristine'>${text}</span></span> `;
